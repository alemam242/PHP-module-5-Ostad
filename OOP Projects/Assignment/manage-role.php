...
if(isset($_SESSION['user'])){
    ...
    if(isset($_POST['deleteUser'])){
        echo '<script>confirm("Do you want to delete this record?")</script>';
        $userId = $_POST['userId'];
        deleteUser($userId);
        header("Location: ./manage-role.php");
    }
                    ...
                    if(!($oneUser['username'] === $user['username'])){
                        ...
                        <td>
                            <form method='POST' action='".$_SERVER['PHP_SELF']."'>
                                ...
                                <select class='form-select-sm' id='editRole' name='role' required>";
                            foreach($roles as $role){
                                if(strtolower($role) === $oneUser['role']){
                                    echo "<option value='{$oneUser['role']}' selected>$role</option>";
                                }else{
                        ...
                        echo "</select>
                                <button type='submit' class='btn btn-warning btn-sm' name='editRole'>Update</button>
                            </form>
                        </td>
                        <td>
                            <form method='POST' onsubmit='return confirmDelete();'>
                                <input type='hidden' name='userId' value='{$oneUser['id']}'>
                                <button class='btn btn-danger btn-sm' type='submit' name='deleteUser'>Delete</button>
                            </form>
                        </td>
                        </tr>";
                    }else{
